...
var uploadsMap = /* @__PURE__ */ new Map();
setInterval(() => {
  const now = Date.now();
  const maxAge = 48 * 60 * 60 * 1e3;
  for (const [id, file] of uploadsMap.entries()) {
    if (now - file.createdAt > maxAge) {
      uploadsMap.delete(id);
      const filePath = import_path.default.join(UPLOADS_DIR, id);
...
\u2022 Light Board (Flex, Backlit, Double Metal Frame, Steel Plate, L-shape Channel): Rs 320 - 360 / sq.ft
\u2022 Forex Board & Sticker: Rs 230 - 280 / sq.ft
\u2022 QR Stand: Rs 300 / pc
\u2022 Photo Frame: Starting from Rs 350 / pc (varies as per frame size and design)
\u2022 Hotel Menu Card: Contact / Ask staff for design options, page count & custom quote
\u2022 Token of Love / Award Stand: Contact / Ask staff for custom quote (depends on size, design, product & manufacturer material)
\u2022 Gift Items: Contact / Ask staff for custom quote (as per design, size & material)
...
\u2022 State that prices are estimate ranges and final price depends on quantity, size, material GSM, frame size, and finishing.
\u2022 For custom staff items (Hotel Menu Card, Token of Love, Gift items, One Way Vision), kindly guide the customer to share their details or click "Forward to Team" on WhatsApp so staff can provide exact quotes.
...
app.post("/api/upload", (req, res) => {
  try {
    ...
    const filePath = import_path.default.join(UPLOADS_DIR, diskFileName);
    ...
    uploadsMap.set(diskFileName, record);
    const protocol = req.headers["x-forwarded-proto"] || req.protocol || "https";
    const host = req.headers.host || `localhost:${PORT}`;
    const downloadUrl = `${protocol}://${host}/api/download/${diskFileName}/${encodeURIComponent(fileName)}`;
    return res.json({
      success: true,
      fileId: diskFileName,
      fileName,
...
app.get("/api/download/:fileId/:fileName?", (req, res) => {
  try {
    ...
    let fileRecord = uploadsMap.get(fileId);
    if (!fileRecord) {
      const filePath = import_path.default.join(UPLOADS_DIR, fileId);
      if (import_fs.default.existsSync(filePath)) {
        const buffer = import_fs.default.readFileSync(filePath);
        const originalName = req.params.fileName || "artwork-file";
        fileRecord = {
          id: fileId,
